...
namespace ICSharpCode.AvalonEdit
{
	...
	public class TextEditor : Control, ITextEditorComponent, IServiceProvider, IWeakEventListener
	{
        ...
        {
            TextArea textArea = this.TextArea;
            textArea.Document.Replace(offset, length, newText);
        }
		...
		/// </summary>
		public void Select(int start, int length, bool isFindOnPageSelection = false)
		{
			...
			textArea.Selection = new SimpleSelection(start, start + length, isFindOnPageSelection);
			textArea.Caret.Offset = start + length;

            if (isFindOnPageSelection)
            {
                DocumentLine line = this.Document.GetLineByOffset(start + length);
                this.ScrollTo(line.LineNumber, start + length - line.Offset);
            }
		}
		...
		public void ScrollToLine(int line)
		{
			...
			ScrollTo(line, -1);            
            this.CaretOffset = Document.Lines[line].Offset;
		}
